...
/** A single fire-seal team card. Meant to be used inside a row/grid, not full-page. */
export function FireSealBadge({
    ...
    <div className="fbc-root inline-block overflow-visible">
      <div className="fbc-group relative" style={{ width: 360 * scale, height: 500 * scale }}>
        <div style={{ transform: `scale(${scale})`, transformOrigin: "top center" }}>
          ...
          <div
            className="fbc-glow pointer-events-none absolute left-1/2 top-[38%] -z-10 h-[280px] w-[280px] -translate-x-1/2 -translate-y-1/2 rounded-full opacity-70 blur-2xl transition-all duration-500"
          ...
          >
            ...
            >
              ...
              >
                ...
                <div className="flex h-full flex-col items-center pt-11 px-8 pb-9">
                  {/* photo */}
                  ...
                  <h1
                    className="mb-2 text-center text-[30px] font-normal uppercase leading-[0.92] tracking-wide"
                    style={{
 ...
 * TeamInfiniteScroll
 * ------------------------------------------------------------------
 * Pass an array of member objects (same shape as FireSealBadge props)
 * and this renders them as a seamless, auto-scrolling marquee row.
 * Hover pauses the scroll so people can read/click a card.
 *
 * <TeamInfiniteScroll members={team} speed={40} />
 *
 * speed = seconds for one full loop (lower = faster).
 * ------------------------------------------------------------------
 */
export default function TeamInfiniteScroll({ members, speed = 40, gap = 40, cardScale = 1 }) {
  ...
  return (
    <div className="w-full overflow-hidden  py-16">
      <style>{`
        ...
        .fbc-track { animation: fbc-marquee linear infinite; }
        .fbc-track-pause:hover .fbc-track { animation-play-state: paused; }
      `}</style>

      <div className="fbc-track-pause">
        <div
          className="fbc-track flex w-max"
          style={{ gap, animationDuration: `${speed}s` }}
        >
          {track.map((member, i) => (
            <FireSealBadge key={`${member.name}-${i}`} {...member} scale={member.scale ?? cardScale} />
          ))}
        </div>
      </div>
    </div>
